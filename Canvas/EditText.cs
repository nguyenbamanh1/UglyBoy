...
{
    public class EditText : ICanvas, IActionListener
    {
		public bool isFocus;
		...
		public EditText(mScreen parentScr)
		{
			text = string.Empty;
		...
		public EditText(string text, int maxLen, int inputType)
		{
			this.text = text;
		...
		public void init()
		{
			width = (GameCanvas.w < 200) ? 140 : 160;
		...
		private void keyPressedAscii(int keyCode)
		{
			...
			{
				char c = (char)keyCode;
				string str = this.text.Substring(0, this.caretPos);
				bool flag3 = this.text.Length > 0;
				if (flag3)
				{
					char befor = this.text[this.text.Length - 1];
					char[] dau = Unikey.getDau(keyCode, befor);
					bool flag4 = dau.Length > 1;
					if (flag4)
					{
						str = this.text.Substring(0, this.caretPos - 1) + dau[0].ToString() + dau[1].ToString();
					...
					{
						str = this.text.Substring(0, this.caretPos - 1) + dau[0].ToString();
				...
				{
					str = this.text.Substring(0, this.caretPos) + c.ToString();
					bool flag7 = this.caretPos < this.text.Length;
					if (flag7)
					{
						str += this.text.Substring(this.caretPos, this.text.Length - this.caretPos);
					}
		...
		{
			//g.setClip(x, y, GameCanvas.w, GameCanvas.h);
			mFont.tahoma_7_white.drawString(g, caption, x + 5, y - mFont.tahoma_7.getHeight(), 0);
			bool flag = isFocused();
			if (inputType == INPUT_TYPE_PASSWORD)
			{
				paintedText = passwordText;
		...
		{
			if (GameCanvas.isPointerHoldIn(x + width - 20, y - SetupView.gI().myCanvas.cmy, 40, height))
			{
				clearAllText();
				isFocus = true;
				SetupView.text = this;
			}
			else if (GameCanvas.isPointerHoldIn(x, y -SetupView.gI().myCanvas.cmy, width - 20, height))
			{
				setFocusWithKb(true);
				SetupView.text = this;
			}
